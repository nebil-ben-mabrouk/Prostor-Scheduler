...
The Embedded REST Server communicates via the port '8181' on the Nimbus host.  This port can be modified by simply editing the file '/src/main/resources/Scheduler.properties', and changing the line "rest.resources.port=8181".


## Integrating Prostor-Scheduler in Storm Nimbus
...
2- Declare the scheduler in the nimbus configuration file ''STORM-HOME/conf/storm.yaml', by adding the following line:
...
3- Check whether the scheduler is correctly running by calling the embedded REST server from a web browser: 
   http://'nimbus-host':8181/log/global  (this address returns the scheduler logs).


## Configuring the components (to be scheduled) in Storm topologies  
...
### Getting scheduling information of a given component
Call the embedded REST server from a web browser: 
http://'nimbus-host':8181/schedule/'topology-id'/'component-id'

An example of a REST Client that calls Prostor-Scheduler is provided in [Prostor-Stream-Groupings](https://github.com/nebil-ben-mabrouk/Prostor-Stream-Groupings)
